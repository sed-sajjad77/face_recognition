        ...
        while cap.isOpened():
            ...
            if cv2.waitKey(1) & 0XFF == ord('a'):
                ...
                if i < 40:
                    ...
                    imgname = os.path.join(self.dir_train, '{}.jpg'.format(uuid.uuid1()))
                    # Write out train image
                    cv2.imwrite(imgname, frame)

                elif i < 55:
                    # Create the unique file path 
                    imgname = os.path.join(self.dir_val, '{}.jpg'.format(uuid.uuid1()))
                    # Write out val images
                    cv2.imwrite(imgname, frame)

                elif i < 65:
                    # Create the unique file path 
                    imgname = os.path.join(self.dir_test, '{}.jpg'.format(uuid.uuid1()))
